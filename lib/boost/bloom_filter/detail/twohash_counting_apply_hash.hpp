...
struct BloomOp {
	...
	BloomOp(const typename CBF::value_type& t, size_t *hash_value1, size_t *hash_value2)
	: hash1_val(hash1(t)),
	  hash2_val(hash2(t))
	{
		*hash_value1 = hash1_val;
		*hash_value2 = hash2_val;
	}

	BloomOp(size_t hash_value1, size_t hash_value2, const typename CBF::value_type& t)
	: hash1_val(hash_value1),
	  hash2_val(hash_value2)
	{
	}
	...
	bool check(const typename CBF::bucket_type& slots,
			const size_t num_bins, uint64_t val)
	{
		for (size_t i = 0; i < N; ++i) {
			const size_t hash =
					(hash1_val + i * hash2_val + ext(i)) % num_bins;
			...
			if (target_bits == 0 || (target_bits == val))
				return false;
		}
...
struct twohash_counting_apply_hash
{
	...
	static void insert(const typename CBF::value_type& t, uint64_t val,
			typename CBF::bucket_type& slots,
			const size_t num_bins, size_t hash_value1, size_t hash_value2)
	{
		BloomOp<N, CBF> inserter(hash_value1, hash_value2, t);
		inserter.update(val, slots, num_bins,
				(static_cast<size_t>(1) << CBF::bits_per_bin()) - 1);
	}
	...
	static bool contains(const typename CBF::value_type& t, uint64_t val,
			...
			const size_t num_bins, size_t *hash_value1, size_t *hash_value2)
	{
		BloomOp<N, CBF> checker(t, hash_value1, hash_value2);
		return checker.check(slots, num_bins, val);

	}

	static bool contains(const typename CBF::value_type& t, uint64_t val,
			const typename CBF::bucket_type& slots,
			size_t hash_value1, size_t hash_value2, const size_t num_bins)
	{
		BloomOp<N, CBF> checker(hash_value1, hash_value2, t);
		return checker.check(slots, num_bins, val);
