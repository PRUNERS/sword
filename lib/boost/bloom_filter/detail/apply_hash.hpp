...
struct apply_hash
{
	...
	static bool contains(const value_type& t,
			const bitset_type& _bits, std::vector<size_t> &hash_values)
	{
		typedef typename boost::mpl::at_c<hash_function_type, N>::type Hash;
		static Hash hasher;

		hash_values[N] = hasher(t) % _bits.size();
		return (_bits[hash_values[N]] &&
				apply_hash<N-1, Container>::contains(t, _bits, hash_values));
	}
...
struct apply_hash<0, Container>
{
			...
			const bitset_type& _bits, std::vector<size_t> &hash_values)
	{
		...
		hash_values[0] = hasher(t) % _bits.size();
		return (_bits[hash_values[0]]);
	}
};
