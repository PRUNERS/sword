...
// #include "swordrt_balanced_bintree.h"
...
struct node
{
	...
	AccessSize size;
	AccessType type;
	node() { address = 0; count = 0; size = size4; type = none; };
	node(size_t a, size_t c, AccessSize s, AccessType t) { address = a; count = c; size = s; type = t; }
};
...
// thread_local node *binAccesses = NULL;
thread_local std::unordered_map<uint64_t, node> binAccesses;
#elif NOTLS
