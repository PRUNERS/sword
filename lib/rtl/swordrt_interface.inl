...
void __swordomp_read8(void *addr, uint64_t hash) {
	...
	std::unordered_map<uint64_t, AccessInfo>::const_iterator item = binAccesses.find(hash);
	if(item == binAccesses.end()) {
		binAccesses.insert(std::make_pair(hash, AccessInfo(access, 1, size8, unsafe_read)));
	} else {
...
void __swordomp_write8(void *addr, uint64_t hash) {
	...
	std::unordered_map<uint64_t, AccessInfo>::const_iterator item = binAccesses.find(hash);
	if(item == binAccesses.end()) {
		binAccesses.insert(std::make_pair(hash, AccessInfo(access, 1, size8, unsafe_write)));
	} else {
		if(access < item->second.address) {
			binAccesses[hash].address = access;
			binAccesses[hash].count++;
