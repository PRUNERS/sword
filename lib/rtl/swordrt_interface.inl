...
void __swordomp_read1(void *addr) {
	if(__swordomp_is_critical__)
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size1, mutex_read);
	else
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size1, unsafe_read);
}

void __swordomp_read2(void *addr) {
	if(__swordomp_is_critical__)
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size2, mutex_read);
	else
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size2, unsafe_read);
}

void __swordomp_read4(void *addr) {
	if(__swordomp_is_critical__)
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size4, mutex_read);
	else
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size4, unsafe_read);
}

void __swordomp_read8(void *addr) {
	if(__swordomp_is_critical__)
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size8, mutex_read);
	else
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size8, unsafe_read);
}
// READS
...
void __swordomp_write1(void *addr) {
	if(__swordomp_is_critical__)
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size1, mutex_write);
	else
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size1, unsafe_write);
}

void __swordomp_write2(void *addr) {
	if(__swordomp_is_critical__)
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size2, mutex_write);
	else
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size2, unsafe_write);
}

void __swordomp_write4(void *addr) {
	if(__swordomp_is_critical__)
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size4, mutex_write);
	else
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size4, unsafe_write);
}

void __swordomp_write8(void *addr) {
	if(__swordomp_is_critical__)
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size8, mutex_write);
	else
		swordRT->CheckMemoryAccess((((size_t) addr)), CALLERPC, size8, unsafe_write);
}
// WRITES
